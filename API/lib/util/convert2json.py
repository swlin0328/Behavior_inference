...
def to_json(dict, json_filename):
	...
	"""
	ENCODING = 'utf-8'
	JSON_NAME = RESULT_DIR + json_filename + '.json'
	raw_data = {}
...
def df2json(df, json_filename, orient='records'):
	...
	Return :
		JSON string.
	"""
	df.to_json(RESULT_DIR + json_filename + '.json', orient=orient, force_ascii=False)
...
def convert_json2data(filename, filename_extension=".svg"):
	...
	for key, value in raw_data.items():
		file_path = RESULT_DIR + key + filename_extension
...
def combine_json(filename1, filename2, append_child=None):
	file_in1 = open(RESULT_DIR + filename1 + '.json', "r")
	file_in2 = open(RESULT_DIR + filename2 + '.json', "r")
	file_out = open(RESULT_DIR + 'combine.json', "w")
