...
from lib.util import user_load_data
from lib.util import csv_process

def cluster_pipe(dataset='row_dataset'):
	# 讀取原始 .csv 檔
	dataSet = csv_process.load_dataset(dataset+'.csv')
	dataSet = dataSet.rename(index=str, columns={"reporttime": "Reporttime", "reportTime": "Reporttime", "channelId": "channelid"})
	...
	return dict2json({'result': True}, 'result4cluster_pipeline')
...
def pipeline(dataSet, pipeline='cluster'):
	return pipe[pipeline](dataSet)
