...
def build_model(training_dataset, config_file):
    ...
    config = {'input': data_layer, 'hidden': hidden_layer, 'output': output_layer, 'optimizer': optimizer, 'init': init,
              'cost': cost, 'model_name': model_config['model']['name'], 'keep_prob': keep_prob, 'batch_size': batch_size}
    return config
